...
public void OnPluginStart()
{
	...
	g_cVipsOnly 			= CreateConVar("sm_c4bomb_vipsonly", 		"0", 		"1 = Only VIPs (with GENERIC FLAG) can plant a bomb | 0 = Public command (Default 0)");
	...
	g_cTeamRestrict 		= CreateConVar("sm_c4bomb_team_restrict", 	"1", 		"1 = All teams | 2 = Only CTs | 3 = Only Terrorist (Default 1)");
...
public Action Event_OnRoundStart(Event event, char[] name, bool dontBroadcast)
{
	...
	{
		g_bRestricted = true;
		g_hStoreTimer = CreateTimer(g_cRestrictRoundStart.FloatValue, Timer_UnlockBomb, _, TIMER_FLAG_NO_MAPCHANGE);
	}
}
